...
Scope-driven differences: explicit intro and required sport/specialization sequence; separate first/last name fields for the existing identity model; result content summarizes the selected profile/goal without claiming a free trial, generated program, BMI, forecast, or official Performance Index. The result page has since been replaced by the PF5 pre-account page, which shows the 7-day free trial offer and the BMI: see [Discovery PF5](discovery-pf5.md). Health measurements and injury questions are not part of the initial seven-question configuration. The five renderer types remain available for backend configuration.
