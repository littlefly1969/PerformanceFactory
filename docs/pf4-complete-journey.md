...
Duration is also stored as `program_duration_weeks` in `UserOnboardingAssessment.profileJson`, which the existing AI cycle/training generation consumes. This slice stops at the saved duration; it introduces no checkout, home, daily sessions or coaching.
...
Local login and public entry route PF4 athletes back to `/journey`. The old `/register/google/consents` route redirects to the PF4 continuation.
