...
type Ability = {
  ...
  isScheduled: boolean; retryScheduled: boolean; errorCode: string | null;
  plan: {
    id: string; version: number; items: { id: string; title: string; body: string; status: string }[]; checkInId: string | null;
    window: { startsOn: string; endsOn: string | null; sessions: number; completed: number; nextDate: string | null } | null;
  } | null;
};
...
export default function AbilitiesPage() {
  ...
  return <AthleteShell label="Abilità" loading={!data} error={error} reload={reload}>
    ...
    {data?.abilities.some(a => !a.isScheduled && ["EMPTY", "ERROR"].includes(a.status)) && <button className="pf4-cta" disabled={busy} onClick={prepare}>{busy ? "Invio richiesta…" : "Prepara i piani mancanti"}</button>}
    ...
    {data?.abilities.map(a => <section className="pf4-athlete-section pf4-ability-card" key={a.id} aria-label={a.name}>
      ...
      {a.isScheduled && a.status === "EMPTY" && <p>Le sedute arrivano con la finestra del programma sportivo, nei giorni liberi da allenamento.</p>}
      ...
      {a.plan && <>
        {a.plan.window
          ? <p>Finestra dal {displayDate(a.plan.window.startsOn)}{a.plan.window.endsOn ? ` al ${displayDate(a.plan.window.endsOn)}` : ""} · {a.plan.window.completed}/{a.plan.window.sessions} sedute fatte</p>
        ...
        <div className="pf4-ability-actions">
          {a.plan.window && <Link href={`/user/training${a.plan.window.nextDate ? `?date=${a.plan.window.nextDate}` : ""}`}>Vedi le sedute nel calendario →</Link>}
