    ...
    <div className="pf-radar-wrap">
      <svg className="pf-radar" viewBox="0 0 300 300" role="img" aria-label="Performance spider chart">
        ...
        {areas.map((area, index) => {
          ...
          const label = pointFor(index, areas.length, max, max, radius + 22, center);
          return (
            <g key={area.id}>
              <line className="pf-radar-axis" x1={center} y1={center} x2={outer.x} y2={outer.y} />
              <text className="pf-radar-label" x={label.x} y={label.y}>
                {area.label.length > 14 ? `${area.label.slice(0, 12)}...` : area.label}
              </text>
            </g>
          );
        })}
