...
export class OrchestratorService {
  ...
  private async runTrainingPlan(
    ...
    return this.prisma.$transaction(async (tx) => {
      const trainingContext = await this.loadTrainingCoachContext(tx, userId);
      ...
      const trainingPlan = await tx.trainingPlanRelease.create({
        data: {
          ...
          sourceSnapshotId: previousSnapshot?.id ?? null,
          proposedByAdminId: actorId,
          items: { create: this.buildTrainingPlanItems(proposal) },
        },
        select: { id: true, version: true },
      });
      ...
      const questionSet = await tx.trainingQuestionSet.create({
        data: {
          ...
          type: 'AI_TRAINING',
          status: 'PENDING_APPROVAL',
          questions: { create: this.buildTrainingQuestions(proposal) },
          approvals: {
            create: {
  ...
  private buildTrainingPlanItems(
    proposal: CycleProposal,
  ...
  private buildTrainingQuestions(
    proposal: CycleProposal,
  ...
  private async loadTrainingCoachContext(
    tx: Prisma.TransactionClient,
    userId: string,
  ) {
