...
export class AiTuningService {
  ...
  async exportActivePrompts(now = new Date()) {
    ...
    this.appendSection(
      ...
      goalPromptConfigs.map((config) =>
        this.renderExportEntry({
          ...
          warning: this.missingActiveVersionWarning(
            config.activePromptVersionId,
          ),
    ...
    const filename = `active-ai-prompts-${this.formatExportTimestamp(now)}.txt`;
    return {
      filename,
      content: `${lines.join('\n')}\n`,
      counts,
  ...
  private renderExportEntry(entry: ExportPromptEntry) {
    return [
      `Type: ${entry.type}`,
      `ID: ${entry.id}`,
  ...
  private missingActiveVersionWarning(activeVersionId?: string | null) {
